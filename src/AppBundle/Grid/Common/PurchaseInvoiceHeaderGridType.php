...
class PurchaseInvoiceHeaderGridType extends DataGridType
{
    ...
    public function buildData(DataBuilder $builder, ObjectRepository $repository, array $options)
    {
        ...
        if (array_key_exists('supplier', $options)) {
            $criteria->andWhere($criteria->expr()->eq('supplier', $options['supplier']));
        }
        if (array_key_exists('mode', $options)) {
            if ($options['mode'] === 'payment') {
                ...
                $criteria->andWhere($criteria->expr()->eq('SIZE(purchaseReceiptDetails)', 0));
            }
        }
